...
for col in df.columns:
    msg = 'columnn : {:>10}\t count of NaN value : {:.0f}'.format(
        col, 100 * (df[col].isnull().sum()))
    print(msg)
...
numeric_cols = ['Daily Time Spent on Site',
...
Categorical_cols = ['Ad Topic Line', 'City',
...
px.histogram(df, x='Daily Time Spent on Site', color='Clicked on Ad',
             template="none", color_discrete_sequence=px.colors.sequential.Darkmint)
...
px.histogram(df, x='Daily Internet Usage', color='Clicked on Ad',
             template="none", color_discrete_sequence=px.colors.sequential.Teal)
...
city_dailytime = pd.DataFrame(df.groupby(
    'City')['Daily Time Spent on Site'].mean().sort_values(ascending=False)[:20])
city_dailytime = city_dailytime.rename_axis(['City']).reset_index()
city_dailytime
px.bar(city_dailytime, x='City', y='Daily Time Spent on Site',
       template="none", color_discrete_sequence=px.colors.sequential.Redor)
...
city_areaincome = pd.DataFrame(df.groupby(
    'City')['Area Income'].mean().sort_values(ascending=False)[:20])
city_areaincome = city_areaincome.rename_axis(['City']).reset_index()
city_areaincome
px.bar(city_areaincome, x='City', y='Area Income', template="none",
...
city_avginternet = pd.DataFrame(df.groupby(
    'City')['Daily Internet Usage'].mean().sort_values(ascending=False)[:20])
city_avginternet = city_avginternet.rename_axis(['City']).reset_index()
city_avginternet
px.bar(city_avginternet, x='City', y='Daily Internet Usage',
       template="none", color_discrete_sequence=px.colors.sequential.Tealgrn)
...
pd.crosstab(df['Country'], df['Clicked on Ad']).sort_values(
    by=1, ascending=False).head(10)
...
city_avginternet = pd.DataFrame(df.groupby(
    'Country')['Daily Internet Usage'].mean().sort_values(ascending=False)[:20])
city_avginternet = city_avginternet.rename_axis(['Country']).reset_index()
city_avginternet
px.bar(city_avginternet, x='Country', y='Daily Internet Usage',
       template="none", color_discrete_sequence=px.colors.sequential.Sunsetdark)
...
city_dailytime = pd.DataFrame(df.groupby(
    'Country')['Daily Time Spent on Site'].mean().sort_values(ascending=False)[:20])
city_dailytime = city_dailytime.rename_axis(['Country']).reset_index()
city_dailytime
px.bar(city_dailytime, x='Country', y='Daily Time Spent on Site',
       template="none", color_discrete_sequence=px.colors.sequential.Magenta)
...
city_areaincome = pd.DataFrame(df.groupby(
    'Country')['Area Income'].mean().sort_values(ascending=False)[:20])
city_areaincome = city_areaincome.rename_axis(['Country']).reset_index()
city_areaincome
px.bar(city_areaincome, x='Country', y='Area Income', template="none",
...
# Sums all 1's i.e clicked for each hour
hdf.style.background_gradient(cmap=cm)
...
sns.countplot(x='Hour', data=df[df['Clicked on Ad']
              == 1], hue='Male', palette='mako')
...
plt.figure(figsize=(17, 6))
sns.countplot(
    x='Weekday', data=df[df['Clicked on Ad'] == 1], hue='Male', palette='YlGnBu')
...
# sns.factorplot(x="Weekday", y="Age", col="Clicked on Ad", data=df,
#                kind="box", size=5, aspect=2.0, palette='BrBG')
...
pd.crosstab(df["Clicked on Ad"], df["Hour"]).T.plot(
    style=[], ax=ax[0], cmap='Set2')
pd.pivot_table(df, index=['Weekday'], values=['Clicked on Ad'], aggfunc=np.sum).plot(
    kind='bar', ax=ax[1], cmap='Set2')  # 0 - Monday
plt.tight_layout()
sns.despine(left=True)
plt.show()
...
f, ax = plt.subplots(1, 2, figsize=(14, 5))
sns.despine(left=True)
# Overall distribution of Males and females count
sns.countplot(x='Male', hue='Clicked on Ad',
              data=df, palette='Paired', ax=ax[0])
table = pd.crosstab(df['Weekday'], df['Clicked on Ad'])
table.div(table.sum(1).astype(float), axis=0).plot(
    kind='bar', stacked=True, ax=ax[1], grid=False, cmap="tab20c")  # 0 - Monday
...
plt.figure(figsize=(17, 6))
sns.countplot(
    x='Month', data=df[df['Clicked on Ad'] == 1], hue='Male', palette='GnBu')
...
# sns.factorplot('Month', 'Clicked on Ad', hue='Male',
#                data=df, palette='PuBu', size=5, aspect=2.5)
# sns.despine(left=True)
# plt.show()
...
f, ax = plt.subplots(1, 2, figsize=(14, 5))
df['Month'][df['Clicked on Ad'] == 1].value_counts(
).sort_index().plot(ax=ax[0], cmap="tab10")
ax[0].set_title('Months Vs Clicks')
ax[0].set_ylabel('Count of Clicks')
pd.crosstab(df["Clicked on Ad"], df["Month"]).T.plot(
    kind='bar', ax=ax[1], cmap="tab10")
...
plt.figure(figsize=(14, 6))
df[df['Clicked on Ad'] == 1].groupby(
    'Date').count()['Clicked on Ad'].plot(cmap="Dark2")
...
# ax = sns.kdeplot(df['Daily Time Spent on Site'], df['Age'],
#                  cmap="Reds", shade=True, shade_lowest=False)
# ax = sns.kdeplot(df['Daily Internet Usage'], df['Age'],
...
fig, axes = plt.subplots(figsize=(17, 6))
# KDE plot for Daily Time Spent on Site
sns.kdeplot(
...
# KDE plot for Daily Internet Usage
sns.kdeplot(
...
# Customize the plot
plt.xlabel('Time')
plt.text(20, 20, "Daily Time Spent on Site", size=16, color='r')
plt.text(200, 60, "Daily Internet Usage", size=16, color='b')
sns.despine(left=True)
plt.show()
...
world_cloud = WordCloud(width=1400, height=700,
...
sns.pairplot(df, hue='Clicked on Ad', vars=['Daily Time Spent on Site', 'Age',
             'Area Income', 'Daily Internet Usage'], palette='mako', height=2, aspect=1.5)
...
# sns.jointplot(df["Daily Time Spent on Site"],
#               df.Age, kind='kde', color='#57CC99')
...
cmap = sns.cubehelix_palette(
    as_cmap=True, start=0, dark=0, light=3, reverse=True)
...
df.groupby('Clicked on Ad')[['Daily Time Spent on Site',
                             'Age', 'Area Income', 'Daily Internet Usage']].mean()
...
count = count.rename(columns={'Male': 'count'})  # Ensure 'count' is lowercase
...
px.box(df, y='Daily Time Spent on Site',
       color='Clicked on Ad', template="none")
...
# Plot 1: Gender and Age vs Clicked on Ad or not
sns.violinplot(x="Male", y="Age", hue="Clicked on Ad",
               data=df, ax=ax[0, 0], palette="spring")
ax[0, 0].set_title('Gender and Age vs Clicked on Ad or not')
ax[0, 0].set_yticks(range(0, 80, 10))

# Plot 2: Weekday and Age vs Clicked on Ad or not
sns.violinplot(x="Weekday", y="Age", hue="Clicked on Ad",
               data=df, ax=ax[0, 1], palette="summer")
ax[0, 1].set_title('Weekday and Age vs Clicked on Ad or not')
ax[0, 1].set_yticks(range(0, 90, 10))

# Plot 3: Gender and Daily Time Spent on Site vs Clicked on Ad or not
sns.violinplot(x="Male", y="Daily Time Spent on Site",
               hue="Clicked on Ad", data=df, ax=ax[1, 0], palette="autumn")
ax[1, 0].set_title('Gender and Daily time spent vs (Clicked on ad or not)')

# Plot 4: Weekday and Daily Time Spent on Site vs Clicked on Ad or not
sns.violinplot(x="Weekday", y="Daily Time Spent on Site",
               hue="Clicked on Ad", data=df, ax=ax[1, 1], palette="winter")
...
plt.figure(figsize=(17, 6))
sns.violinplot(x=df['Male'], y=df['Area Income'], data=df,
               palette='viridis', hue='Clicked on Ad')
...
px.scatter(df, y='Daily Time Spent on Site', x='Age',
           color='Clicked on Ad', template="none")

# In[71]:


px.scatter(df, y='Area Income', x='Age',
           color='Clicked on Ad', template="none")

# In[72]:


px.scatter(df, y='Daily Internet Usage', x='Age',
           color='Clicked on Ad', template="none",)
...
px.scatter(df, y='Daily Internet Usage', x='Daily Time Spent on Site',
           color='Clicked on Ad', template="none")

# In[74]:


px.scatter(df, y='Daily Internet Usage', x='Area Income',
           color='Clicked on Ad', template="none")
...
analysis_cols = ['Daily Time Spent on Site',
                 'Age', 'Area Income', 'Daily Internet Usage']
...
for col in analysis_cols:
    ...
    w, p = stats.mannwhitneyu(
        x=clicked, y=non_clicked, alternative='two-sided')
