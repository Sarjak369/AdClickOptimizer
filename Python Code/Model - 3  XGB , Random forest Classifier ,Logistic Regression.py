...
# In[5]:
X = df.drop(['Timestamp', 'Clicked on Ad',
            'Ad Topic Line', 'Country', 'City'], axis=1)
y = df['Clicked on Ad']

X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.3, random_state=42)

num_columns = ['Daily Time Spent on Site', 'Age',
...
# Assuming print_score is defined elsewhere
print_score(lr_clf, X_train, y_train, X_test, y_test, train=True)
print_score(lr_clf, X_train, y_train, X_test, y_test, train=False)
...
coefdf = pd.DataFrame(data=X_train.columns, index=range(
    0, len(lrCoef[0])), columns=['Feature'])
...
# Barplot
sns.barplot(data=coefdf, y=coefdf['Feature'],
            x=coefdf['Coef'], palette='winter')
...
# Save the figure
plt.savefig('Coefficient of Logistic Regression.png')  # Removed fontsize here
