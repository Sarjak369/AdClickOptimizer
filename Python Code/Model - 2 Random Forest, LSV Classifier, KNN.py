...
plt.style.use('seaborn-v0_8-whitegrid')  # Correct style name
...
rf_clf = RandomForestClassifier(
    n_estimators=min(best_n_estimators), random_state=0)
...
rf_fpr, rf_tpr, rf_thresholds = roc_curve(
    y_valid, rf_clf.predict_proba(X_valid)[:, 1])
...
plt.plot(svc_lin_fpr, svc_lin_tpr,
         label=f'Linear Support Vector Classifier: {round(svc_lin_auc, 3)}')
