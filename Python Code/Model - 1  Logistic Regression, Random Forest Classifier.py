...
pp = sns.pairplot(new_df, hue='Clicked on Ad', vars=[
                  'Month', 'Day', 'Hour', 'Weekday'], palette='mako', height=2, aspect=1.5)
...
new_df = pd.concat([new_df, pd.get_dummies(
    new_df['Month'], prefix='Month')], axis=1)
# Dummy encoding on weekday column
new_df = pd.concat([new_df, pd.get_dummies(
    new_df['Weekday'], prefix='Weekday')], axis=1)
...
new_df = pd.concat([new_df, pd.get_dummies(
    new_df['Hour_bins'], prefix='Hour')], axis=1)
...
sns.barplot(x=new_df['Age'], y=df['Clicked on Ad'], ci=None, palette='GnBu')
plt.xticks(rotation=90)
plt.title("Barplot of Age vs Clicked on Ad")
plt.xlabel("Age")
...
plt.axvspan(-1, x_limit_1, alpha=0.25, color='green', label='Under 18')
plt.axvspan(x_limit_1, x_limit_2, alpha=0.25, color='red', label='18-35')
plt.axvspan(x_limit_2, 50, alpha=0.25, color='yellow', label='Above 35')

# Add labels, legend, and rotate x-ticks
plt.xticks(rotation=90)
...
new_df['Age_bins'] = pd.cut(new_df['Age'], bins=[0, 18, 30, 45, 70], labels=[
                            'Young', 'Adult', 'Mid', 'Elder'])
...
sns.countplot(x='Age_bins', hue='Clicked on Ad', data=new_df,
              palette='PuBu')  # Use keyword arguments
plt.title('Count of Clicked on Ad by Age Bins')
plt.xlabel('Age Bins')
plt.ylabel('Count')
plt.xticks(rotation=90)
plt.show()
...
new_df = pd.concat([new_df, pd.get_dummies(
    new_df['Age_bins'], prefix='Age')], axis=1)
...
new_df = pd.concat([new_df, pd.get_dummies(
    new_df['Country'], prefix='Country')], axis=1)
...
X = new_df.drop(['Clicked on Ad'], axis=1)  # Specify axis=1 to drop columns
...
print("Accuracy of Logistic regression training set:",
      round(lr_training_prediction, 3))
...
sns.heatmap(confusion_matrix(y_train, y_pred),
            annot=True, fmt='3.0f', cmap="tab20b")
plt.title('Confusion_matrix', y=1.05, size=15)
...
sns.barplot(x="importance", y='index',
            data=feature_importances[0:10], label="Total", color="b", palette='Paired')
...
rf_fpr, rf_tpr, rf_thresholds = roc_curve(
    y_test, rf.predict_proba(X_test_std)[:, 1])
